...
# Header
echo -e "${BLUE}===============================================${NC}"
echo -e "${BLUE}  Validação de Velocidade UFABCnet - v1.2${NC}"
echo -e "${BLUE}===============================================${NC}"
...
for i in $(seq 1 $TESTS); do
    ...
    if [ "$result" != "null" ]; then
        ...
        echo -e "${RED}✗ Falhou (3 tentativas)${NC}"
    fi
...
# Função para teste de upload com retry
retry_upload_test() {
    local max_retries=3
...
# =============================================================================
# TESTE DE LATÊNCIA (PING ÚNICO COM 12 PACOTES)
# =============================================================================
echo -e "${GREEN}🏓 Iniciando teste de Latência...${NC}"
echo -ne "   Executando $TESTS pings... "

# Executar ping único com todos os testes
ping_result=""
...
if [ $? -eq 0 ]; then
    ...
    if [ -n "$ping_times" ]; then
        ...
        latency_values=($ping_times)
        
        if [ ${#latency_values[@]} -gt 0 ]; then
            # Calcular média dos tempos (mais apropriado para ping batch)
            total_latency=0
            for time in "${latency_values[@]}"; do
                total_latency=$(echo "$total_latency + $time" | bc)
            done
            latency_average=$(echo "scale=3; $total_latency / ${#latency_values[@]}" | bc)
            
            echo -e "${GREEN}✓ Concluído${NC}"
            echo -e "   ${BLUE}📊 Latência - Média: ${latency_average} ms (${#latency_values[@]}/$TESTS pings bem-sucedidos)${NC}"
        else
    ...
    latency_average="0.000"
    echo -e "${RED}✗ Falhou${NC}"
    echo -e "   ${RED}❌ Latência - Comando ping falhou${NC}"
fi
...
echo -e "   4. O teste de latência usa média de $TESTS pings"
echo 